...
```{r}
.libPaths()
newpath <- c("K:/EPIC_NORFOLK/XX/Library")
.libPaths(newpath)
.libPaths()
```
...
```{r}
#Separating day from date

dff <- dff%>%
  mutate(Day=yday(NDATE))
  
dff$Day <- as.numeric(dff$Day)

dff$Day <- rescale(dff$Day)
...
fit <- lm(LENTILQUANT_SUM~sin(2*3.1416*Day)+cos(2*3.1416*Day), data=dff)
summary(fit)
...
cox_S <- coxph(dff.surv~legume_rankea+sin(2*3.1416*Day)+cos(2*3.1416*Day))
summary(cox_S)


cox_SC <- coxph(dff.surv~legumesvge+sin(2*3.1416*Day)+cos(2*3.1416*Day))
summary(cox_SC)
...
cox_MS1 <- coxph(Surv(entryage, exitage, DM_INC08)~legume_rankea+
                  sin(2*3.1416*Day)+cos(2*3.1416*Day)+
                  SEX)
summary(cox_MS1)
...
```{r}
## Truncating 99.5 percentile of data
quantile(dff$Total_legumes, 0.95)

dr <- dff%>%
  filter(Total_legumes<=39.97)
```
...
```{r}
## Fit the model
legume_rankr <- factor(dr$legume_rankr, levels=c(0,1,2,3))
SEX <- factor(dr$SEX, levels=c(1,2))
CIGSTAT <- factor(dr$CIGSTAT, levels=c(1,2,3))
RUPE <- factor(dr$RUPE, levels=c(1,2,3,4))
SC <- factor(dr$SC, levels=c(0,10,20,31,32,40,50))
EDLEVEL09 <- factor(dr$EDLEVEL09, levels=c(0,1,2,3))
FH_DM <- factor(dr$FH_DM, levels=c(1,2))
FH_CVA <- factor(dr$FH_CVA, levels=c(1,2))
FH_MI <- factor(dr$FH_MI, levels=c(1,2))
FH_CA <- factor(dr$FH_CA, levels=c(1,2))
WAIST_Class <- factor(dr$WAIST_Class, levels=c(1,2))
...
matplot(lterm$x, lterm$HR, type='l', lty = 1, xlab = 'Consumption of total legumes(servings/day)', 
        ylab='Hazard ratio', xlim=c(0,2), ylim=c(0,2))
